...
import (
	...
	"github.com/harche/runvm/libcontainer"
	"github.com/harche/runvm/libcontainer/cgroups/systemd"
	...
	"github.com/harche/runvm/hypervisor"
...
func newProcess(p specs.Process) (*libcontainer.Process, error) {
	p.Args = []string{"tail" , "-f", "/etc/hosts"}
...
func (r *runner) run(config *specs.Process) (int, error) {
	...
	for _, mount := range r.container.Config().Mounts {
		...
		if strings.HasSuffix(mount.Source, "resolv.conf") {
			vmParams.ResoveString, _ = ioutil.ReadFile(mount.Source)
			s := string(vmParams.ResoveString)
			if strings.Contains(s, "127.0.0.11") {
				skipHostFile = true
				vmParams.HostsString, _ = ioutil.ReadFile("/etc/hosts")
			}
		}

		if strings.HasSuffix(mount.Source, "hosts") && skipHostFile == false {
			vmParams.HostsString, _ = ioutil.ReadFile(mount.Source)
		}
	}
